...
To create a new resource with a View page, you can use the--viewflag:
...
By default, the View page will display a disabled form with the record’s data. If you preferred to display the record’s data in an “infolist”, you can define aninfolist()method on the resource class:
...
Thecomponents()method is used to define the structure of your infolist. It is an array ofentriesandlayout components, in the order they should appear in your infolist.

Check out the Infolists docs for aguideon how to build infolists with Filament.

## #Adding a View page to an existing resource

If you want to add a View page to an existing resource, create a new page in your resource’sPagesdirectory:
...
If your resource is simple, you may wish to view records in modals rather than on theView page. If this is the case, you can justdelete the view page.

If your resource doesn’t contain aViewAction, you can add one to the$table->recordActions()array:
...
You may wish to modify the data from a record before it is filled into the form. To do this, you may define amutateFormDataBeforeFill()method on the View page class to modify the$dataarray, and return the modified version before it is filled into the form:
...
Alternatively, if you’re viewing records in a modal action, check out theActions documentation.
...
Users may access the View page if theview()method of the model policy returnstrue.

## #Creating another View page

One View page may not be enough space to allow users to navigate a lot of information. You can create as many View pages for a resource as you want. This is especially useful if you are usingresource sub-navigation, as you are then easily able to switch between the different View pages.

To create a View page, you should use themake:filament-pagecommand:
...
Now, you can define theinfolist()orform()for this page, which can contain other components that are not present on the main View page:
...
This is useful when you havemultiple view pagesand need different relation managers on
each page:
...
IfgetAllRelationManagers()isn’t defined, any relation managers defined in the resource will be used.
...
Each page in Filament has its ownschema, which defines the overall structure and content. You can override the schema for the page by defining acontent()method on it. Thecontent()method for the View page contains the following components by default:
...
This assumes that you have created a view atresources/views/filament/resources/users/pages/view-user.blade.php:
