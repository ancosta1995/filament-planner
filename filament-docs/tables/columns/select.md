...
**URL:** https://filamentphp.com/docs/5.x/tables/columns/select  
**Section:** tables  
**Page:** select  
**Priority:** high  
...
If you have lots of options and want to populate them based on a database search or other external data source, you can use thegetOptionsSearchResultsUsing()andgetOptionLabelUsing()methods instead ofoptions().

ThegetOptionsSearchResultsUsing()method accepts a callback that returns search results in$key => $valueformat. The current user’s search is available as$search, and you should use that to filter your results.
...
BothgetOptionsSearchResultsUsing()andgetOptionLabelUsing()must be used on the select if you want to provide custom search results:
...
You may employ theoptionsRelationship()method of theSelectColumnto configure aBelongsTorelationship to automatically retrieve options from. ThetitleAttributeis the name of a column that will be used to generate a label for each option:
...
By default, when usingoptionsRelationship(), Filament will remember the options for the duration of the table page to improve performance. This means that the options function will only run once per table page instead of once per cell. You can disable this behavior using therememberOptions(false)method:
...
### #Valid options validation (inrule)

Theinrule ensures that users cannot select an option that is not in the list of options. This is an important rule for data integrity purposes, so Filament applies it by default to all select fields.
...
If you are using theoptionsRelationship()method, thegetOptionLabelUsing()method will be automatically defined for you, so you don’t need to worry about it.
